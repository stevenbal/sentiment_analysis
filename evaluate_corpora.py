...
positive_path = argumentparser.positive_path(N, words, stemming,
                                             stopword_removal, training_corpus)
negative_path = argumentparser.negative_path(N, words, stemming,
                                             stopword_removal, training_corpus)

# Create the n-gram models with the given settings and save them to the correct
# paths
if method == 'create':
    start = time()
    training_corpus_pos = f'{corpora_dir}{training_corpus}/positive_training.csv'
    training_corpus_neg = f'{corpora_dir}{training_corpus}/negative_training.csv'
    LM_pos = ngram.LanguageModel('positive', training_corpus_pos, N=N,
                                 words=words, stemming=stemming,
                                 stopword_removal=stopword_removal)
    LM_neg = ngram.LanguageModel('negative', training_corpus_neg, N=N,
                                 words=words, stemming=stemming,
                                 stopword_removal=stopword_removal)
    end = time()
    LM_pos.save_models(positive_path)
    LM_neg.save_models(negative_path)
    logging.info(f'Model construction for {training_corpus} took {end-start} \
                   seconds')
...
def save_results(result_filename, training_corpus, **kwargs):
    ...
    -training_corpus:   str, name of the corpus used to train the models
                        with
...
save_results('NaiveBayesClassifier_results.csv', training_corpus, N=N,
