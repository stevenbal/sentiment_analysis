...
positive_path = argumentparser.positive_path(
    N, words, stemming, stopword_removal, train_corpus
)
...
precision, recall, counts = classifier.evaluate(
    dev_data, mixture=mixture, prediction_thres=0
)
end = time()
...
save_results(
    "NaiveBayesClassifier_results.csv",
    train_corpus,
    N=N,
    words=words,
    stemming=stemming,
